...
def StockCryptFiles(root):
    ...
    frameStockCF = ctk.CTkFrame(master=root, fg_color="#f8fafc", bg_color="#f8fafc")
    frameStockCF.pack(fill=tk.BOTH, expand=True)

    # Navbar Frame
    NavBar = ctk.CTkFrame(master=frameStockCF, fg_color="#fff", bg_color="#fff", width=1400, height=60)
    NavBar.pack()
    ...
    button_back = ctk.CTkButton(master=frameStockCF, text="", image=image_back, hover=False,
                                compound="top", cursor="hand2", bg_color="transparent",
                                fg_color="transparent", command=lambda: back(frameStockCF))
    ...
    image_label_info = ctk.CTkLabel(frameStockCF, image=image_info, text='')
    image_label_info.place(x=1065, y=100)
    label_download_key = ctk.CTkLabel(master=frameStockCF,
    ...
    label_df = ctk.CTkLabel(master=frameStockCF,
                            text=".......................................",
    ...
    frame_tv = ctk.CTkFrame(frameStockCF, fg_color='#DAE5F4', width=1300, height=500)
    ...
    query = f"Select * from Fichiers_Crypte where id_user='{ss.current_user}'"
    ...
    if result:
        ...
        for row in result:
            file_name = row[1]
            date_reparation = row[2].strftime("%d/%m/%Y")
            Cle = row[3]
            print(data)
    ...
    frameStockCF.bind("<Button-1>", lambda event: ss.on_user_interaction(root, frameStockCF))
